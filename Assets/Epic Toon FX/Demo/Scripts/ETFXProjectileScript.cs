...
public class ETFXProjectileScript : MonoBehaviour {
  ...
  void FixedUpdate() {
    ...
    Vector2 direction = transform.GetComponent<Rigidbody2D>().velocity; // Gets the direction of the projectile, used for collision detection
    // Debug.Log("dir: " + direction);

    transform.GetComponent<Rigidbody2D>().AddForce (new Vector2(-1f, 1f) * 9.81f, ForceMode2D.Force);
    direction = direction.normalized;

    var detectionDistance = transform.GetComponent<Rigidbody2D>().velocity.magnitude * Time.deltaTime; // Distance of collision detection for this frame
